...
void Ws2812UpdateHand(int position, uint8_t index, uint8_t series)
{
  position = (position + Settings.string_rotation[series]) % Settings.string_pixels[series];
...
void Ws2812Clock(void)
{
  ...
  for (byte series = 0; series < Settings.light_strings; series++) {
    int pixelcnt = (int)Settings.string_pixels[series];
    if( pixelcnt < 1 || pixelcnt > WS2812_MAX_LEDS) { // avoid divby0
      pixelcnt = (int)Settings.light_pixels;
    }
    int clksize = 60000 / pixelcnt;
